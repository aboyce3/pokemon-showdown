...
export class BattleQueue extends Array<Action> {
	...
	sort(DO_NOT_USE?: unknown) {
		if (DO_NOT_USE) throw new Error(`Battle queues can't be sorted with a custom comparator`);
	...
	/**
	 * `this.slice()` returns a BattleQueue instance, strangely.
	 */
	toJSON() {
		return [...this];
	}
